    ...
    if (event.type == EVENT_KEY_DOWN)
    {
        ...
        if (data->slot == INPUT_SLOT_CMDLINE)
        {
                ...
                !m_movieLock && !m_editLock && !m_cmdEdit)
            {
                ...
                m_cmdEdit = true;
                m_cmdHistoryCurIdx = -1; //cmd history index to -1, because we not browsing history initia
            }
            return false;
        }
...
void CRobotMain::ExecuteCmd(const std::string& cmd)
{
    if (cmd.empty())
    {
        m_cmdCommand = false; //it was not command!
        return;
    }
