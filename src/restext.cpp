...
//?		if ( num == ERR_COM             )  strcpy(text, "Kommunikationsproblem mit dem Roboter");
	...
	if ( type == RES_TEXT )
	{
		...
		if ( num == RT_TITLE_EDITOR        )  strcpy(text, "Edytor poziomÛw");
	...
	if ( type == RES_EVENT )
	{
		...
		if ( num == EVENT_INTERFACE_EDITOR )  strcpy(text, "Edytor\\Edytor poziomÛw");
		...
		if ( num == EVENT_INTERFACE_DOWNLOAD)  strcpy(text, "Pobierz\\Pobierz wiÍciej poziomÛw z internetu");
