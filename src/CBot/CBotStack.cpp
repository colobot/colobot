...
{
    if ( m_error>=0 )
        return false;                // normal output
    if ( m_error==CBotError(-3) )
        return false;            // normal output (return current)
...
{
    if (n != CBotNoErr && m_error != CBotNoErr)
...
bool CBotStack::Execute()
{
    ...
    if ( instr == nullptr )
        return true;                // normal execution request

    if (!instr->Run(nullptr, pile))
        return false;            // resume interrupted execution
...
void CBotStack::AddVar(CBotVar* pVar)
{
    ...
    while (p != nullptr && p->m_block == BlockVisibilityType::INSTRUCTION)
        p = p->m_prev;
...
void CBotStack::GetRunPos(std::string& functionName, std::size_t& start, std::size_t& end)const
{
    ...
    while (p->m_next != nullptr)
    {
        ...
        if (p->m_func == IsFunction::YES && p->m_instr != nullptr )
            funct = p->m_instr;
        if ( p->m_next->m_prog != prog )
            break ;

        if (p->m_next2 && p->m_next2->m_state != 0)
            p = p->m_next2 ;
        else
            p = p->m_next;
    }
...
////////////////////////////////////////////////////////////////////////////////
CBotVar* CBotStack::GetStackVars(std::string& functionName, int level)const
{
    const CBotProgram*    prog = m_prog;    // current program
    functionName = "";
    ...
    const CBotStack* pp = p;
    while ( pp != nullptr )
    {
        if (pp->m_func == IsFunction::YES)
            break;
        pp = pp->m_prev;
    }
...
bool CBotStack::SaveState(FILE* pf)
{
    ...
    {
        if (!WriteWord(pf, 2))
            return false; // a marker of type (m_next2)
        if (!m_next2->SaveState(pf))
            return false; // saves the next element
    }
    else
    {
        if (!WriteWord(pf, 1))
            return false; // a marker of type (m_next)
    }
    if (!WriteWord(pf, static_cast<unsigned short>(m_block)))
        return false;
    if (!WriteWord(pf, m_state))
        return false;
    if (!WriteWord(pf, 0))
        return false; // for backwards combatibility (m_bDontDelete)
    if (!WriteWord(pf, m_step))
        return false;

    if (!SaveVars(pf, m_var))
        return false;            // current result
    if (!SaveVars(pf, m_listVar))
        return false;        // local variables

    if (m_next != nullptr)
    {
        if (!m_next->SaveState(pf))
            return false; // saves the next element
    }
    else
    {
...
bool SaveVars(FILE* pf, CBotVar* pVar)
{
    while (pVar != nullptr)
    {
        if (!pVar->Save0State(pf))
            return false; // common header
        if (!pVar->Save1State(pf))
            return false; // saves the data
...
bool CBotStack::RestoreState(FILE* pf, CBotStack* &pStack)
{
    ...
    if ( w == 2 ) // 2 - m_next2
    {
        if (!pStack->RestoreState(pf, pStack->m_next2))
            return false;
    }
    ...
    if (!CBotVar::RestoreState(pf, pStack->m_var))
        return false;    // temp variable
    if (!CBotVar::RestoreState(pf, pStack->m_listVar))
        return false;// local variables
...
{
    if (!WriteWord(pf, 100+static_cast<int>(m_mPrivate)))
        return false;        // private variable?
    if (!WriteWord(pf, m_bStatic))
        return false;                // static variable?
    if (!WriteWord(pf, m_type.GetType()))
        return false;        // saves the type (always non-zero)

    if (m_type.Eq(CBotTypPointer) && GetPointer() != nullptr)
    {
        if (GetPointer()->m_bConstructor)                    // constructor was called?
        {
            if (!WriteWord(pf, (2000 + static_cast<unsigned short>(m_binit)) ))
                return false;
            return WriteString(pf, m_token->GetString());    // and variable name
        }
    }

    if (!WriteWord(pf, static_cast<unsigned short>(m_binit)))
        return false;          // variable defined?
    return WriteString(pf, m_token->GetString());            // and variable name
}
...
bool CBotVar::RestoreState(FILE* pf, CBotVar* &pVar)
{
    ...
    while ( true )            // retrieves a list
    {
        ...
        if ( w == 200 )
        {
            if (!ReadString(pf, defnum))
                return false;            // number with identifier
            if (!ReadWord(pf, w))
                return false;                    // type
        }
...
//                if (!ReadString(pf, s)) return false;
                {
                    ...
                    pNew = new CBotVarClass(token, r);                // directly creates an instance
                                                                    // attention cptuse = 0
                    if ( !RestoreState(pf, (static_cast<CBotVarClass*>(pNew))->m_pVar))
                        return false;
                    pNew->SetIdent(id);
        ...
        case CBotTypNullPointer:
            if (!ReadString(pf, s))
                return false;   // name of the class
            {
...
//                if ( id ) p = CBotVarClass::Find(id);        // found the instance (made by RestoreInstance)
                ...
                if ( !CBotVar::RestoreState( pf, pInstance ) )
                    return false;
                (static_cast<CBotVarPointer*>(pNew))->SetPointer( pInstance );            // and point over

                if (bConstructor)
                    pNew->ConstructorSet(); // constructor was called
                if (ptrType.Eq(CBotTypPointer))
                    pNew->SetType(ptrType); // keep pointer type
...
bool CBotStack::IsCallFinished()const
{
