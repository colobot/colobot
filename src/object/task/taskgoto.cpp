...
bool CTaskGoto::EventProcess(const Event &event)
{
                ...
                for (int x = 0; x < m_bmSize; x++)
                {
                    for (int y = 0; y < m_bmSize; y++)
                    {
                        ...
                        if (a || b)
                        {
                            ...
                            if (b)
                                c = Gfx::Color(0.0f, 0.0f, 1.0f, 1.0f);
    ...
    if ( m_phase == TGP_BEAMGOTO )  // goto dot list? (?)
    {
        ...
        if ( cirSpeed < -1.0f )
            cirSpeed = -1.0f;
        if ( dist < 4.0f )
            cirSpeed *= dist/4.0f;  // so close -> turns less
...
Error CTaskGoto::IsEnded()
{
    ...
    {
        if ( m_altitude != 0.0f
            && (m_object->Implements(ObjectInterfaceType::JetFlying)
            && dynamic_cast<CJetFlyingObject*>(m_object)->GetReactorRange() < 0.1f) )  // overheating?
        {
            m_physics->SetMotorSpeedX(0.0f);  // stops the advance
            m_physics->SetMotorSpeedZ(0.0f);  // stops the rotation
    ...
    if ( m_phase == TGP_CRWAIT )  // waits after collision?
    {
        ...
        if ( m_time >= 1.0f )
        {
            if ( m_crashMode == TGC_RIGHTLEFT ||
                 m_crashMode == TGC_RIGHT     )
                angle =  Math::PI/2.0f;  // 90 degrees to the right
            else
                angle = -Math::PI/2.0f;  // 90 degrees to the left
...
//?         m_phase = TGP_ADVANCE;
    ...
    if ( m_phase == TGP_CLWAIT )  // waits after collision?
    {
        if ( m_time >= 1.0f )
        {
            if (m_crashMode == TGC_RIGHTLEFT)
                angle = -Math::PI;
            if (m_crashMode == TGC_LEFTRIGHT)
                angle =  Math::PI;
            if (m_crashMode == TGC_RIGHT)
                angle =  Math::PI/2.0f;
            if (m_crashMode == TGC_LEFT)
                angle = -Math::PI/2.0f;
            m_angle = Math::NormAngle(m_object->GetRotationY()+angle);
            m_phase = TGP_CLTURN;
        }
...
bool CTaskGoto::AdjustBuilding(Math::Vector &pos, float margin, float &distance)
{
    for (CObject* obj : CObjectManager::GetInstancePointer()->GetAllObjects())
    {
        ...
        Math::Vector oPos;
        float suppl = 0.0f;
        if ( !GetHotPoint(obj, oPos, false, 0.0f, suppl) )
            continue;
        float dist = Math::DistanceProjected(pos, oPos);
        if ( dist <= margin )
        {
                            ...
                            bool bTake, float distance, float &suppl)
{
    ...
    if ( type == OBJECT_DERRICK )
    {
        mat = pObj->GetWorldMatrix(0);
        pos.x += 8.0f;
        if ( bTake && distance != 0.0f )
            suppl = 4.0f;
        if ( bTake )
            pos.x += TAKE_DIST+distance+suppl;
        pos = Transform(*mat, pos);
        return true;
    }
    ...
    if ( type == OBJECT_RESEARCH )
    {
        ...
        if ( bTake && distance != 0.0f )
            suppl = 2.5f;
        if ( bTake )
            pos.x += TAKE_DIST+TAKE_DIST_OTHER+distance+suppl;
        pos = Transform(*mat, pos);
        return true;
    }
...
bool CTaskGoto::LeakSearch(Math::Vector &pos, float &delay)
{
    ...
    if (min > crashSphere.radius + obstacleCrashSphere.radius + 4.0f)
        return false;
...
void CTaskGoto::ComputeRepulse(Math::Point &dir)
{
    ...
    if ( iType == OBJECT_WORM || iType == OBJECT_CONTROLLER )
        return;
    ...
    for (CObject* pObj : CObjectManager::GetInstancePointer()->GetAllObjects())
    {
        ...
        for (const auto& crashSphere : pObj->GetAllCrashSpheres())
        {
            ...
            if ( oPos.y-oRadius > iPos.y+iRadius )
                continue;
            if ( oPos.y+oRadius < iPos.y-iRadius )
                continue;
...
void CTaskGoto::BeamStart()
{
    ...
    max = m_goal;
    if ( min.x > max.x )
        Math::Swap(min.x, max.x);
    if ( min.z > max.z )
        Math::Swap(min.z, max.z);
                            ...
                            float goalRadius)
{
    ...
    if (step < BM_DIM_STEP*2.1f)
        step = BM_DIM_STEP*2.1f;
    if (step > 20.0f)
        step = 20.0f;
                             ...
                             int i, int nbIter)
{
    ...
    iLar = 0;
    if ( i >= MAXPOINTS )
        return ERR_GOTO_ITER;  // too many recursions
    ...
    if ( m_bmIter[i] == -1 )
    {
        ...
        {
            if ( !BitmapTestLine(prevPos, curPos, angle/nbDiv, true) )
    ...
    if ( iLar >= m_bmIter[i] )
    {
        ...
        if ( ret != ERR_GOTO_IMPOSSIBLE )
            return ret;
        m_bmIter[i] = iLar+1;
        for ( iClear=i+1 ; iClear<=MAXPOINTS ; iClear++ )
            m_bmIter[iClear] = -1;
        m_bmIterCounter ++;
        if ( m_bmIterCounter >= nbIter )
            return ERR_CONTINUE;
    }
    iLar ++;
                               ...
                               float angle, float step)
{
    ...
    float goalAngle = Math::RotateAngle(goalPoint.x-startPoint.x,
                                        goalPoint.z-startPoint.z);
