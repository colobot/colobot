...
void CPhysics::FloorAngle(const Math::Vector &pos, Math::Vector &angle)
{
    ...
    a1 = atanf(m_terrain->GetFloorHeight(pw)/character->wheelFront);

    pw.x = pos.x+character->wheelBack*cosf(angle.y+Math::PI*1.0f);
    pw.y = pos.y;
    pw.z = pos.z-character->wheelBack*sinf(angle.y+Math::PI*1.0f);
    a2 = atanf(m_terrain->GetFloorHeight(pw)/character->wheelBack);
    ...
    a1 = atanf(m_terrain->GetFloorHeight(pw)/character->wheelLeft);
    ...
    a2 = atanf(m_terrain->GetFloorHeight(pw)/character->wheelRight);
