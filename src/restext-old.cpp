...
//?		if ( num == EVENT_OBJECT_MAP       )  strcpy(text, "Mini-map");
	...
	if ( type == RES_EVENT )
	{
		...
		if ( num == EVENT_INTERFACE_GADGET )  strcpy(text, "Nombre d'objets dÈcoratifs\\QualitÈ d'objets non indispensables");
...
//?		if ( num == EVENT_OBJECT_MAP       )  strcpy(text, "Mini-carte");
	...
	if ( type == RES_TEXT )
	{
		...
		if ( num == RT_TITLE_USER          )  strcpy(text, "Poziom uøytkownika");
		...
		if ( num == RT_PLAY_CHAPu          )  strcpy(text, " Poziom uøytkownika:");
		...
		if ( num == RT_DIALOG_YESQUIT      )  strcpy(text, "ZakoÒcz\\KoÒczy grÍ COLOTOT");
		...
		if ( num == RT_DIALOG_DELGAME      )  strcpy(text, "Czy na pewno chcesz skasowaÊ grÍ gracza %s? ");
...
//?		if ( num == RT_GENERIC_EDIT2       )  strcpy(text, "www.?.com");
	...
	if ( type == RES_EVENT )
	{
		...
		if ( num == EVENT_INTERFACE_USER   )  strcpy(text, "Poziom\\Poziomy uøytkownika");
		...
		if ( num == EVENT_INTERFACE_QUIT   )  strcpy(text, "ZakoÒcz\\KoÒczy grÍ COLOTOT");
		...
		if ( num == EVENT_INTERFACE_INVERTX)  strcpy(text, "OdwrÛcenie myszy X\\OdwrÛcenie kierunkÛw przewijania na w poziomie");
		if ( num == EVENT_INTERFACE_INVERTY)  strcpy(text, "OdwrÛcenie myszy Y\\OdwrÛcenie kierunkÛw przewijania na w pionie");
		if ( num == EVENT_INTERFACE_EFFECT )  strcpy(text, "Wstrzπsy przy wybuchach\\Ekran trzÍsie siÍ podczas wybuchÛw");
		if ( num == EVENT_INTERFACE_MOUSE  )  strcpy(text, "CieÒ myszy\\Dodaje cieÒ kursorowi myszy");
		if ( num == EVENT_INTERFACE_EDITMODE) strcpy(text, "Automatyczne wciÍcia\\Podczas edycji programu");
		...
		if ( num == EVENT_INTERFACE_KPROG  )  strcpy(text, "PodrÍcznik programowania");
		...
		if ( num == EVENT_INTERFACE_SILENT )  strcpy(text, "Cisza\\Bez düwiÍkÛw");
		...
		if ( num == EVENT_OBJECT_RTANK     )  strcpy(text, "Rozpocznij prace badawcze robotÛw na gπsienicach");
		if ( num == EVENT_OBJECT_RFLY      )  strcpy(text, "Rozpocznij prace badawcze robotÛw latajπcych");
		...
		if ( num == EVENT_OBJECT_RTOWER    )  strcpy(text, "Rozpocznij prace badawcze wieøy obronnej");
		...
		if ( num == EVENT_OBJECT_RiPAW     )  strcpy(text, "Rozpocznij prace badawcze robotÛw na nogach");
...
//?		if ( num == EVENT_OBJECT_MAP       )  strcpy(text, "Mapka");
	...
	if ( type == RES_ERR )
	{
		...
		if ( num == INFO_RESEARCHTANK   )  strcpy(text, "DostÍpne plany robotÛw na gπsienicach");
		...
		if ( num == INFO_DELETEANT      )  strcpy(text, "MrÛwka úmiertelnie ranna");
		if ( num == INFO_DELETEBEE      )  strcpy(text, "Osa úmiertelnie ranna");
		if ( num == INFO_DELETEWORM     )  strcpy(text, "Robal úmiertelnie ranny");
		if ( num == INFO_DELETESPIDER   )  strcpy(text, "Pajπk úmiertelnie ranny");
	...
	if ( type == RES_CBOT )
	{
		...
		if ( num == TX_WHILE         ) strcpy(text, "SkrÛt klawiszowy ""while"" missing");
		if ( num == TX_BREAK         ) strcpy(text, "Polecenie ""break"" outside a loop");
		if ( num == TX_LABEL         ) strcpy(text, "Po etykiecie musi wystπpiÊ ""for"", ""while"", ""do"" or ""switch""");
	...
	if ( type == RES_KEY )
	{
		...
		if ( num == VK_BACK             )  strcpy(text, "<--");
		if ( num == VK_TAB              )  strcpy(text, "Tab");
		if ( num == VK_CLEAR            )  strcpy(text, "WyczyúÊ");
		if ( num == VK_RETURN           )  strcpy(text, "Enter");
		if ( num == VK_SHIFT            )  strcpy(text, "Shift");
		if ( num == VK_CONTROL          )  strcpy(text, "Ctrl");
		if ( num == VK_MENU             )  strcpy(text, "Alt");
